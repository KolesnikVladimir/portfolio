...
function fileInput(fi_container_class, fi_button_class, fi_filename_class, fi_button_text) {
	...
	fi_button_text		=	fi_button_text		||	'Прекрепить файл...'; // Text inside the button
	...
	// String to append
	var fi_str = '<div class="'+fi_container_class+'"><div class="'+fi_button_class+'">'+fi_button_text+'</div><div class="'+fi_filename_class+'">в файле .doc, .pdf, .docx ...</div></div>';
		...
		$('.'+fi_container_class).each(function() {
			...
			if(fi_real_name == fi_fake_name) {
				...
				if(fi_file_name){
					...
					$('.'+fi_container_class+'[data-name='+fi_real_name+'] .'+fi_filename_class).html("в файле .doc, .pdf, .docx или .rtf");
